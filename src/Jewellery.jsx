...
const Jewellery = () => {
  ...
  const [sortType, setSortType] = useState(0); // 0: Yeniler, 1: Azalan, 2: Artan
  ...
  // Ürün Verisi ve Sıralama Mantığı
  const initialProducts = Array.from({ length: 28 }).map((_, i) => ({
    id: `jew-${i}`,
    name: lang === 'TR' ? `ÖZEL TASARIM MÜCEVHER #${i + 101}` : `EXCLUSIVE JEWELLERY #${i + 101}`,
    price: 12500 + (i * 250), // Sıralama testi için değişken fiyat
    date: i,
    img: `https://picsum.photos/seed/jew-${i}/800/1067`
  }));

  const sortedProducts = [...initialProducts].sort((a, b) => {
    if (sortType === 1) return b.price - a.price; // Fiyat: Azalan
    if (sortType === 2) return a.price - b.price; // Fiyat: Artan
    return b.date - a.date; // En Yeniler
  });
      ...
      <div className="jewellery-page-fluid">
        ...
        <div className="filter-section-container">
          <div className="filter-bar-sticky">
            <div className="filter-bar-content">
              <div className="res-count">{initialProducts.length} {t.results}</div>
              <div className="filter-actions">
                <div className="action-item" onClick={() => setActivePanel('filter')}>
                  <Icon type="filter" /> <span>{t.filter}</span> <Icon type="arrow" />
                </div>
                <div className="action-item" onClick={() => setActivePanel('sort')}>
                  <Icon type="sort" /> <span>{t.sort}</span> <Icon type="arrow" />
                </div>
              </div>
            </div>
          </div>
        ...
        <div className={`side-panel ${activePanel ? 'open' : ''}`}>
          <div className="panel-header">
            <span className="panel-title">{activePanel === 'filter' ? t.filterTitle : t.sortTitle}</span>
            <div onClick={() => setActivePanel(null)} style={{ cursor: 'pointer' }}><Icon type="close" /></div>
          </div>
          <div className="panel-content">
            {activePanel === 'filter' ? (
              ['KATEGORİ', 'MADEN', 'TAŞ TİPİ', 'FİYAT'].map(item => (
          ...
          <div className="panel-footer">
            <button className="btn-apply" onClick={() => setActivePanel(null)}>{t.apply}</button>
          </div>
        </div>
            ...
            <div key={product.id} className="luxury-item-card">
              ...
              <div className="item-details">
                ...
                <div className="price-row">
                  <p className="price-text">{product.price.toLocaleString()} {t.currency}</p>
        ...
        <style>{`
          .jewellery-page-wrapper { width: 100%; min-height: 100vh; }
          .jewellery-page-fluid { width: 100%; background: #fff; }
          .filter-section-container { padding-top: 40px; background: #fff; }
          .filter-bar-sticky { position: sticky; top: 155px; background: #fff; z-index: 100; border-top: 1px solid #f2f2f2; border-bottom: 1px solid #f2f2f2; width: 100%; }
          .filter-bar-content { display: flex; justify-content: space-between; align-items: center; padding: 25px 12%; width: 100%; }
          ...
          .luxury-product-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 40px 20px; padding: 40px 12% 100px; width: 100%; background: #fff; }
          .luxury-item-card { width: 100%; display: flex; flex-direction: column; }
          .item-img-box { width: 100%; aspect-ratio: 3/4; overflow: hidden; background: #f9f9f9; position: relative; }
          .item-img-box img { width: 100%; height: 100%; object-fit: cover; transition: transform 0.8s cubic-bezier(0.165, 0.84, 0.44, 1); }
          .luxury-item-card:hover img { transform: scale(1.06); }
          .wishlist-btn { position: absolute; top: 15px; right: 15px; z-index: 5; opacity: 0.4; transition: 0.3s; cursor: pointer; }
          ...
          .panel-header { padding: 30px; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #f2f2f2; }
          ...
          .btn-apply { width: 100%; padding: 18px; background: #000; color: #fff; border: none; font-size: 11px; font-weight: 700; cursor: pointer; letter-spacing: 2px; text-transform: uppercase; }
        `}</style>
      </div>
    </div>
  );
