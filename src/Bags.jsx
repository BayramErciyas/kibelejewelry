...
const Telkari = () => {
  ...
  const initialProducts = Array.from({ length: 34 }).map((_, i) => ({
    id: `tel-${i}`,
    name: lang === 'TR' ? `EL İŞLEMESİ TELKARİ #${i+1}` : `HANDMADE FILIGREE #${i+1}`,
    price: 4500 + (i * 100), // Fiyat farkı görmek için
    date: i,
    img: `https://picsum.photos/seed/tel-${i}/800/1067`
  }));

  const sortedProducts = [...initialProducts].sort((a, b) => {
    if (sortType === 1) return b.price - a.price;
    if (sortType === 2) return a.price - b.price;
    ...
    <div className="telkari-page-wrapper">
      <div className="telkari-page-fluid">
          ...
          <div className="panel-content">
            {activePanel === 'filter' ? (
              ...
              lt.sortOptions.map((opt, i) => (
                <div key={i} className={`filter-group ${sortType === i ? 'active-sort' : ''}`} onClick={() => { setSortType(i); setActivePanel(null); }} style={{ cursor: 'pointer' }}>
                  <span>{opt}</span>{sortType === i && <Icon type="check" />}
                </div>
              ))
            )}
          </div>
        ...
        <style>{`
          ...
          .filter-group { display: flex; justify-content: space-between; align-items: center; padding: 20px 0; border-bottom: 1px solid #f9f9f9; font-size: 12px; letter-spacing: 1px; }
